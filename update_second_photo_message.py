...
"""
Скрипт для обновления сообщения book_hero_second_photo_request
"""

import sqlite3
from datetime import datetime

def update_second_photo_message():
    """Обновляет сообщение book_hero_second_photo_request с правильным текстом"""
    try:
        conn = sqlite3.connect('bookai.db')
        cursor = conn.cursor()
        
        message_key = "book_hero_second_photo_request"
        message_name = "Запрос второго фото героя"
        content = "Благодарим 🙏🏻\n{sender_name}, отправь ещё одно фото лица, можно с другого ракурса — это сделает иллюстрацию ещё точнее 🎯"
        context = "book"
        stage = "hero_second_photo_request"
...
def main():
    print("📸 Скрипт для обновления сообщения о втором фото героя")
    print("=" * 50)
    
    if update_second_photo_message():
        print("\n✅ Сообщение обновлено!")
        print("🎯 Теперь текст будет правильным с именем пользователя")
    else:
